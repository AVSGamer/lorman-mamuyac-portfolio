...
func _get_cell_data(row: int, column: int) -> Dictionary:
	...
	if row == 0:
		...
		if column == 0:
			...
			return {
				"title": "My Front Case Fans: [url=https://www.coolermaster.com/en-sg/products/sickleflow-140-argb.html][b]Cooler Master 140 ARGB SickleFlow[/b][/url] x2",
				"desc": "140 is the size, there are other sizes:\nMost casing allow attaching of the 2 common sizes on the same spot with allocations of screwholes for them. These are my intake fans at the front inner side of the case.",
		...
		if column == 0:
			...
			return {
				"title": "My Rear Case Fan: [url=https://www.idcooling.com/product/detail?id=159&name=XF-12025-RGB-TRIO][b]ID-COOLING XF-12025-RGB Black[/b][/url]",
		...
		if column == 0:
			...
			return {
				"title": "My Fan Header Splitter: [url=https://www.idcooling.com/product/detail?id=259&name=FS-04%20ARGB][b]ID-COOLING FS04 ARGB[/b][/url]",
				"desc": "Motherboard Fan Port Count:\nThis whole overkill Lighting in PC Cases didn't exist back then so there weren't really much of these connectors and probably still not that many on a board even today. Might need splitters to accomodate plentiful lighting emitters for your PC Case Nightlight.",
		...
		if column == 0:
			...
			return {
				"title": "My Monitor: [url=https://www.samsung.com/us/business/support/owners/product/sa450-series-s24a450bw-1/][b]Samsung SyncMaster 24'' SA450[/b][/url]",
		...
		if column == 0:
			...
			return {
				"title": "My Speakers: [url=https://global.microless.com/product/creative-sbs-a60-2-0-desktop-speaker/][b]Creative SBS A60[/b][/url]",
		...
		if column == 0:
			...
			return {
				"title": "My Mechanical Keyboard: [url=https://rakk.ph/product/rakk-lam-ang-pro-barebone-rgb-mechanical-gaming-keyboard-pbt-doubleshot-white-keycaps-5-pin-mechanical-switch-bundles/][b]Rakk Lam-Ang PRO PBT Barebones[/b][/url] - Some Zilents Switches",
		...
		if column == 0:
			...
			return {
				"title": "Router: [url=https://mikrotik.com/product/RB952Ui-5ac2nD-TC][b]MikroTik HomeAP AC Lite[/b][/url]",
				"desc": "Seems to be the competitor to CISCO:\nThey both offer certifications. That last time I checked, have to renewed with an exam and a payment.",
		...
		if column == 0:
			...
			return {
				"title": "My [url=https://shopee.ph/1080p-Hd-Webcam-Web-Camera-With-Mic-Computer-Pc-Laptop-Skype-Msn-i.9277068.4760523907][b]Webcam[/b][/url]: Some Cheap Low Quality",
				"desc": "No Idea why I even mention this:\nIt's what it's as at least having one I guess.",
