	...
	if row == 0:
		...
		if column == 0:
			return {
				...
				"desc": "System Pipeline Allocation:\nThe CPU dictates your memory frequencies and bus speed layout limits. Secure core throughput targets before mapping tertiary peripherals. Second because it depends on what programs you want to run and it will decide the type of motherboard you'll have to get next because CPUs have different slot size or shapes. The higher in price you go, you can expect the higher the price of the other components will be as well for optimal or avoiding bottlenecks around a certain ratio per class/level of computing. If you ever plan to upgrade your CPU and you are using a legitimate Windows License, either make sure to disable TPM & Bitlocker first or backup/get their keys before upgrading. The TPM/Bitlocker uses your current OS-running setups' motherboard + CPU HardwareIDs or MetaData to create these keys. So when you upgrade your CPU the already installed OS will look for that original CPU and if finds a mismatch with what is saved, will trigger the security and ask for those keys to be able to log back in. If you can't get past the BitLocker or can't put back the old CPU anymore. I've never tried requesting online for that key so I wouldn't know if that would actually work, but wiping all the data instead and reinstalling the OS is what you can do. Re-using the previous license though, not much experience here since when I upgrade my parts. I make sure everything is okay first before letting go of my old parts. Changing Motherboards though that's a tough one if you want to re-use your legit Windows License and see the Motherboard Section why.",
